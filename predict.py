...
def load_csv(path):
    ...
    data=np.array(data)
    return data
def output(model,idx,dataset_path="./ML_DATA"):
...
def super_pose(list_fv,number):  ### this function plot predicted curves superimposed on initial input dispersion image.
    vf=loadtxt("./ML_DATA/%04d/fv_boundary.csv"%number,delimiter=',')
    f=vf[:2]
    v=vf[2:]
    #fig, ax = plt.subplots(figsize=(6,6))
    figure(7,figsize=(6, 6), dpi=80)
    dispersion=loadtxt('./ML_DATA/%04d/dispersion_matrix.csv'%number,delimiter=',')
...
def main(args):
    ...
    number=args.sample
    im,seg,emb,dis_mat,fv=output(model,number)
    ...
    clustering = DBSCAN(eps=0.5, min_samples=50, metric='euclidean').fit(emb_concerned)
