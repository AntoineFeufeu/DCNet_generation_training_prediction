...
begin:
	...
	if ((options = cmdlineproc (argc, argv, "iacEklprRsStdeuhg:z:o:f:v:x:C:q:Q:b:")) != NULL)
	{
		...
		if (output_format == 2)		// CSS
		{
			if (seis_buffer_length > 99999999)
			{
				printf("Warning. For CSS output the maximum seismic buffer length is 99999999. Resetting to this amount.\n");
				seis_buffer_length = 99999999;
			...
			case 'M':
				...
				{ 
							...
							chk_network(input_data_hdr->network))
					{
						...
						if (strncmp(prev_stn, input_data_hdr->station, 5) != 0 ||
                                                	strncmp(prev_chn, input_data_hdr->channel, 3) != 0 ||
                                                	strncmp(prev_loc, input_data_hdr->location, 2) != 0 ||
                                                	strncmp(prev_net, input_data_hdr->network, 2) != 0)
							{
								tspan_flag = TRUE;
							}
						LRECL = get_LRECL(input_data_hdr, lrecord_ptr);
						...
						strncpy(prev_stn, input_data_hdr->station, 5);
						prev_stn[5] = 0;

						strncpy(prev_chn, input_data_hdr->channel, 3);
						prev_chn[3] = 0;

						strncpy(prev_loc, input_data_hdr->location, 2);
						prev_loc[2] = 0;

						strncpy(prev_net, input_data_hdr->network, 2);
						prev_net[2] = 0;
...
********** */
	...
	if (data_flag)
	{
		...
		// warn user if CSS requested that max is 99999999
		if (output_format == 2)
		{
			if (seis_buffer_length > 99999999)
			{
				printf("Warning, for CSS output the maximum seismic buffer length is 999999. Resetting length to 99999999\n");
				seis_buffer_length = 99999999;
			}
