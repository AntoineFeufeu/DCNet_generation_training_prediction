...
CHANGES
	...
	06 MAY 2014 - corrected hue logic starting lines 564 and 575 (Larry Baker USGS Menlo Park)
*/
...
void dv_openpl(int showmenu)
{
	...
	if(dc_rotate){
		dc_Nxpi = (double)dc_right/ 8.00;
		dc_Nypi = (double)dc_top  /10.00;
	} else {
...
void di_gcmdln(int argc,char *argv[])
{
	...
	if(dc_rotate){
		itmp = config.numypixels;
		config.numypixels = config.numxpixels;
		config.numxpixels = itmp;
	}
}
