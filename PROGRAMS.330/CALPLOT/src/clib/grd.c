...
c---------------------------------------------------------------------c
Changes:
	28 APR 2014 Fix lines 496 516 in for loops
		at old 503 for( ; *s = (char )getc(fin); s++) replaced by
		  for ;( *s = (char )getc(fin)); s++)  
		at  523 for(nread=0 ; s[nread] = (char )getc(fin);  nread++, nread < n){
		replaced by for(nread=0 ; (nread < n) && ( s[nread] = (char )getc(fin) );  nread++){
		Thanks to Larry Baker, USGS, Menlo Park
*/
#include <stdio.h>
#include <string.h>
#include <signal.h>
					...
					we only flag negative with a newline */
{
	...
	if(n < 0){	/* single character followed by newline */
		...
		*s = '\0';
		...
		for(nread=0 ; (nread < n ) && (s[nread] = (char )getc(fin));  nread++){
			if(s[nread] == '\n'){
				break;
			}
		}
