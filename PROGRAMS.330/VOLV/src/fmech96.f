...
c       02 JUL 2018 - if the sensor is in the fluid and the source is in the
...
c         gm/cm^3, then the P?? Green's functions output bythe CPS codes`
...
c         using theMIXed units. So for the MIXed units, fmech96 assumes that the output
...
c         for the MIXed units, when the user askes for synthetics with moment Mo,
...
c                   present
c               iftype =  3  jsrc(i),i=1,3 indicates if trace 
c                   is present
...
c               If there are pressure histories available, then
c               we use the iftype=16 instead of the iftype=3
c               then  jsrc(i),i=1,4 indicates
c                   jsrc value: 1=Z, 2=N, 3=E, 4=R, 5=T, 6 = P
c                     jsrc(i),i=5,16 = 0
c
c-----
...
c-----
        ...
        integer jjsrc(21)
        logical lhavez, lhaver, lhavet, lhavep
c-----
...
c PEX PDD PDS PSS PVF PHF
        if(jjsrc(16) .ne. 0)lhavep = .true.
        if(jjsrc(17) .ne. 0)lhavep = .true.
        if(jjsrc(18) .ne. 0)lhavep = .true.
        if(jjsrc(19) .ne. 0)lhavep = .true.
        if(jjsrc(20) .ne. 0)lhavep = .true.
        if(jjsrc(21) .ne. 0)lhavep = .true.
...
c-----
c      output the pressure field
c      to do this the xp is multiplied by 1.0E+04
c      and the sign is changed to convert from stress to pressure
c      to go from moment in dyne-cm to Pa
c-----
                if(jsrc(4).ne.0)then
                    stcomp = 'P       '
                    cmpinc = 0.0
...
C     1 ' -V                   Verbose output'
     ...
     1  ' -h                   This online help'
        write(LER,*)
     1  ' Units: It is assumed that the model file is in units of km,'
        write(LER,*)
     1  '     km/s and gm/cm^3. The output of fmech96 is thus in cm, '
        write(LER,*)
     1  '     or cm/s/s the Z, R, T, N or E components.'
        write(LER,*)
     1  '   If measurements are made in a fluid, then the PEX etc'
        write(LER,*)
     1  '   Green s functions are computed and the output is a P, or'
        write(LER,*)
     1  '   component with units of Pa'
        stop
        end
