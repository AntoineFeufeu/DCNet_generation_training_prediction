...
void gsac_set_param_read(int ncmd, char **cmdstr)
{
	 ...
	 */
	/* for processing history mechanism for prs always save
		important commands */
...
void gsac_exec_read(void)
{
	...
	gsac_control.fft = NO;
	/* ASSUME READ and not READ MORE */
	/* also the trace file is known to be a valid SAC file */
	for(k=0, gsac_control.number_itraces=0, gsac_control.number_iheaders=0, gsac_control.number_otraces=0 ; k < gsac_control.max_number_traces ; k++){
		...
		iret = brsac(sacdata[k].sac_ifile_name,&sacdata[k].sachdr,&sacdata[k].sac_data);
		if(iret >=0){
			gsac_control.number_itraces++;
			gsac_control.number_iheaders++;
			gsac_control.number_otraces++;
			/* set epoch time */
			...
			/* set correct depmax depmin depmen */
